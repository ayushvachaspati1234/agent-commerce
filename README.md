...
- Theme: graphite ink, copper accent, crosshatch hero band, square corners, copper-edged cards (`--ink`, `--accent`, `--bg-hero` in `style.css`).
- Hero visual is a pure HTML/CSS **agent-to-agent transaction thread**: negotiation messages → mandate check chip → dark settlement receipt.
...
- Logo is an inline SVG exchange-arrows mark in `index.html` (nav + footer) — swap both `<svg class="logo-mark">` blocks to change it.
